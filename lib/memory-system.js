(function () {
    ...
    const PERSON_PATTERNS = [
        /\b(?:my|our)\s+(?:friend|sister|brother|mom|dad|mother|father|boyfriend|girlfriend|husband|wife|partner|boss|coworker|colleague|roommate|ex)\s+([A-Z][a-z]+)/,
        /\b([A-Z][a-z]{2,})\s+(?:told me|texted me|called me|said|asked me)/,
        /\b(?:talked to|met with|hung out with|saw)\s+([A-Z][a-z]{2,})\b/
    ];

    const HOBBY_PATTERNS = [
        /\bi\s+(?:love|enjoy|like|adore|can't stop)\s+([a-z][^.!?,\n]{5,40})/i,
        /\bmy\s+(?:hobby|passion|obsession|favourite|favorite)\s+(?:is|was)\s+([^.!?,\n]{5,50})/i,
        /\bi\s+(?:play|watch|listen to|read|do|practice)\s+([^.!?,\n]{4,40})\s+(?:every|all the|most)/i
    ];

    const FAVORITE_PATTERNS = [
        /\bmy\s+(?:favourite|favorite)\s+(?:song|band|album|show|movie|film|book|food|color|colour|drink|game)\s+is\s+([^.!?,\n]{3,50})/i,
