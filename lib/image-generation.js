(function () {
    ...
    function createImageGeneration(api) {
        ...
        function needsContextResolution(userMessage) {
            ...
            // Unambiguous character-subject requests (e.g. "send me a selfie", "take a photo").
            // Even when the stripped directive is empty there is nothing to resolve — the
            // character IS the subject and the legacy path handles these correctly.
            // Routing them through the LLM risks misclassification as 'creative' or 'object',
            // which would drop the character's visual description from the SD prompt entirely.
            const UNAMBIGUOUS_CHARACTER_PATTERNS = [
                /\b(selfie|front[-\s]?cam)\b/i,
                /\b(send|show|share)\s+(me\s+)?(an?\s+)?(selfie|photo|pic|picture|snap|image)\b/i,
                /\btake\s+(me\s+)?(an?\s+)?(selfie|photo|pic|snap)\b/i,
                /\bsnap\s+(me\s+)?(an?\s+)?(photo|pic|picture|selfie|shot)\b/i,
                /\b(photo|pic|picture|selfie|snap|image)\s+of\s+(you|yourself)\b/i,
                /\bgot any (photos?|pics?|pictures?|selfies?)\b/i,
                /\bany (photos?|pics?|pictures?|selfies?) of you\b/i,
            ];
            if (UNAMBIGUOUS_CHARACTER_PATTERNS.some(p => p.test(msg))) return false;
