(function () {
    ...
    function createProactiveMessaging(api) {
        ...
        function buildProactiveContextMsg(lastUserMessageAt, history = []) {
            ...
            // Anti-repetition: show the AI its last 8 outgoing messages (not just 4)
            // and instruct it to avoid the same topic, imagery, and emotional framing —
            // not just different wording of the same idea.
            const recentCharMessages = history
                .filter(m => !m.is_user && m.mes)
                .slice(-8);

            if (recentCharMessages.length > 0) {
                const msgList = recentCharMessages
                    .map(m => `- "${String(m.mes).replace(/\n/g, ' ').slice(0, 180).trim()}"`)
                    .join('\n');
                lines.push(
                    `<recent_outbound_messages>\n${msgList}\n</recent_outbound_messages>`,
                    'Your next message MUST be on a completely different topic from every message above.',
                    'Do not reuse the same metaphors, emotional framing, imagery, or conversation thread. Move somewhere new.'
                );
            }
